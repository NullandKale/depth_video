...
def initialize_output(output_folder, frame_size, fps, video_path, output_name=None, output_mode='file', rtsp_server='127.0.0.1', rtsp_port='8554', pipe_output=False):
    if output_mode == 'file':
        return initialize_file_output_libx264(output_folder, frame_size, fps, video_path, pipe_output, output_name)
        # return initialize_file_output_nvenc(output_folder, frame_size, fps, video_path, pipe_output)
...
def initialize_file_output_libx264(output_folder, frame_size, fps, video_path, pipe_output, output_name):
    ...
    if output_name == None:
        ...
        output_video_filename = os.path.join(output_folder, (output_name + '_output.mp4'))

    print(f"Generating File: {output_video_filename}")
...
def initialize_file_output_nvenc(output_folder, frame_size, fps, video_path, pipe_output, output_name):
    os.makedirs(output_folder, exist_ok=True)
    video_filename = os.path.basename(video_path)
    ...
    output_args = ['-c:v', 'h264_nvenc', '-pix_fmt', 'yuv420p', '-crf', '18', '-maxrate', '8M', '-bufsize', '512M']
    ffmpeg_cmd = ['ffmpeg', '-y', '-f', 'rawvideo', '-pix_fmt', 'rgb24', '-s', f"{frame_size[0]}x{frame_size[1]}", '-r', str(fps), '-i', '-', '-i', video_path, '-c:v', 'copy', '-c:a', 'aac', '-map', '0:v:0', '-map', '1:a:0', '-movflags', 'faststart', '-muxdelay', '0', *output_args, output_video_filename]
    if pipe_output:
        video_writer = subprocess.Popen(ffmpeg_cmd, stdin=subprocess.PIPE)
    else:
